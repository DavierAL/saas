...
describe('Money value object', () => {
  test('createMoney: accepts integer cents', () => {
    expect(() => createMoney(1050, 'PEN')).not.toThrow();
    expect(createMoney(1050, 'PEN').amount).toBe(1050);
  });
  ...
  test('createMoney: zero is valid', () => {
    expect(createMoney(0, 'PEN').amount).toBe(0);
  });

  test('addMoney: adds same currency', () => {
    const a = createMoney(500, 'PEN');
    const b = createMoney(300, 'PEN');
  ...
  test('multiplyMoney: quantity 0 gives 0', () => {
    expect(multiplyMoney(createMoney(500, 'PEN'), 0).amount).toBe(0);
  });

  test('multiplyMoney: quantity 3', () => {
    expect(multiplyMoney(createMoney(500, 'PEN'), 3).amount).toBe(1500);
  });

  test('multiplyMoney: throws for negative quantity', () => {
    expect(() => multiplyMoney(createMoney(500, 'PEN'), -1)).toThrow();
  });
  ...
  test('formatMoney: PEN 1.00', () => {
    expect(formatMoney(createMoney(100, 'PEN'))).toBe('PEN 1.00');
  });

  test('formatMoney: PEN 7.05', () => {
    expect(formatMoney(createMoney(705, 'PEN'))).toBe('PEN 7.05');
  });
});
...
describe('calculateLineSubtotal', () => {
  test('1 unit × 380 cents = 380 cents', () => {
    expect(calculateLineSubtotal({ unit_price: 380, quantity: 1 }, 'PEN').amount).toBe(380);
  });

  test('2 units × 500 cents = 1000 cents', () => {
    expect(calculateLineSubtotal({ unit_price: 500, quantity: 2 }, 'PEN').amount).toBe(1000);
  });

  test('0 units = 0', () => {
    expect(calculateLineSubtotal({ unit_price: 999, quantity: 0 }, 'PEN').amount).toBe(0);
  });

  test('preserves currency', () => {
    const result = calculateLineSubtotal({ unit_price: 500, quantity: 2 }, 'PEN');
...
describe('calculateOrderTotal', () => {
  test('empty order = 0', () => {
    expect(calculateOrderTotal([], 'PEN').amount).toBe(0);
  });

  test('single line', () => {
    const lines = [{ unit_price: 700, quantity: 5 }];
    expect(calculateOrderTotal(lines, 'PEN').amount).toBe(3500);
  });
  ...
  test('multiple lines sum correctly', () => {
    ...
    // 500 + 380 + 1350 = 2230
    expect(calculateOrderTotal(lines, 'PEN').amount).toBe(2230);
  });
  ...
  test('matches manual calculation', () => {
    ...
    expect(calculateOrderTotal(lines, 'PEN').amount).toBe(expected);
  });
});
