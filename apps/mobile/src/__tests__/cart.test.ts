...
import type { CartState } from '@saas-pos/application';
...
describe('Cart Store', () => {
  describe('addItemToCart', () => {
    test('adds new item to empty cart with qty 1', () => {
      const result = addItemToCart(EMPTY_CART, ITEM_A);
    ...
    test('adds new item with custom quantity', () => {
      const result = addItemToCart(EMPTY_CART, ITEM_A, 3);
      expect(result.items[0]!.quantity).toBe(3);
    });

    test('increments quantity if item already in cart', () => {
      const cart1 = addItemToCart(EMPTY_CART, ITEM_A, 2);
  ...
  describe('removeItemFromCart', () => {
    test('removes correct item', () => {
      let cart = addItemToCart(EMPTY_CART, ITEM_A);
    ...
    test('removing non-existent item is a no-op', () => {
      const cart = addItemToCart(EMPTY_CART, ITEM_A);
  ...
  describe('getCartTotal', () => {
    test('returns 0 for empty cart', () => {
      expect(getCartTotal(EMPTY_CART, 'PEN')).toBe(0);
    });

    test('single item total', () => {
      const cart = addItemToCart(EMPTY_CART, ITEM_A, 2);
      expect(getCartTotal(cart, 'PEN')).toBe(500);
    });

    test('multi-item total', () => {
      let cart = addItemToCart(EMPTY_CART, ITEM_A, 2);
      cart = addItemToCart(cart, ITEM_B, 1);
      expect(getCartTotal(cart, 'PEN')).toBe(880);
    });

    test('respects currency', () => {
      const cart = addItemToCart(EMPTY_CART, ITEM_A, 2);
      expect(getCartTotal(cart, 'USD')).toBe(500);
    });
  });
});
